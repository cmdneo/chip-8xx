...
std::optional<std::vector<u8>> Parser::parse_and_assemble()
{
...
static std::optional<u16> limit_value(int value, unsigned bits)
{
...
Result Parser::expect(TokenKind kind, std::optional<int> value)
{
...
bool Parser::match_advance(TokenKind kind, std::optional<int> value)
{
	if (match(kind, value)) {
		advance();
		return true;
...
bool Parser::match(TokenKind kind, std::optional<int> value)
{
...
static Matched match_one(std::string_view rule_token, Token tok)
{
	using M = Matched;
...
Matched RuleMatcher::try_next(Token tok)
{
	if (matched)
		return Matched::None;
	Matched code{};
